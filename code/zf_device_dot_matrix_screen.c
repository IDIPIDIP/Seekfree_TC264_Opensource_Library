...
uint8   dot_matrix_screen_bitmap[3][7];     // 自定义位图显示缓冲区，3个字符位，每个字符位 5x7
uint8   dot_matrix_screen_bitmap_backup[3][7];
uint16  dot_matrix_screen_brightness;       // 屏幕亮度
uint8   dot_matrix_screen_show_bitmap_flag; // 0: 字符显示模式 1: 位图显示模式
...
//左转
const uint8 zuo_zhuan[3][7] =
{
    {0x20, 0x30, 0x38, 0x3C, 0x38, 0x30, 0x20},
    {0x00, 0x00, 0x3E, 0x3E, 0x3E, 0x00, 0x00},
    {0x00, 0x00, 0x3E, 0x3E, 0x3E, 0x00, 0x02}
};

//右转
const uint8 you_zhuan[3][7] =
{
    {0x00, 0x00, 0x3E, 0x3E, 0x3E, 0x00, 0x00},
    {0x00, 0x00, 0x3E, 0x3E, 0x3E, 0x00, 0x00},
    {0x02, 0x06, 0x0E, 0x1E, 0x0E, 0x06, 0x02}
...
//近光
const uint8 jin_guang[3][7] =
{
    {0x20, 0x10, 0x08, 0x08, 0x08, 0x10, 0x20},
    {0x1E, 0x10, 0x10, 0x10, 0x10, 0x10, 0x1E},
    {0x02, 0x04, 0x0A, 0x04, 0x0A, 0x04, 0x08}
};

//远光
const uint8 yuan_guang[3][7] =
{
    {0x20, 0x10, 0x08, 0x08, 0x08, 0x10, 0x20},
    {0x1E, 0x10, 0x10, 0x10, 0x10, 0x10, 0x1E},
...
//雾灯
const uint8 wu_deng[3][7] =
{
    {0x20, 0x10, 0x08, 0x08, 0x08, 0x10, 0x20},
    {0x1E, 0x10, 0x10, 0x10, 0x10, 0x10, 0x1E},
    {0x04, 0x0E, 0x04, 0x0E, 0x04, 0x0E, 0x04}
};

//车内灯
const uint8 che_neideng[3][7] =
{
{0x10, 0x0C, 0x1C, 0x1E, 0x1C, 0x0C, 0x10},
{0x00, 0x00, 0x1C, 0x1C, 0x1C, 0x00, 0x00},
{0x04, 0x18, 0x1C, 0x1E, 0x1C, 0x18, 0x04}
};

//雨刮器
const uint8 yu_guaqi[3][7] =
{
{0x08, 0x12, 0x24, 0x08, 0x12, 0x24, 0x08},
{0x24, 0x08, 0x12, 0x24, 0x08, 0x12, 0x04},
{0x12, 0x24, 0x08, 0x12, 0x24, 0x08, 0x10}
};
...
void dot_matrix_screen_scan(void)
{
    ...
    if(0 == (entry_num % 2))
    {
        ...
        {
            if(dot_matrix_screen_show_bitmap_flag)
            {
                // 位图模式：bitmap[x][row] 的 bit1~bit5 对应每列像素
                for(i = 0; 5 > i; i++)
                {
                    if(dot_matrix_screen_bitmap_backup[0][display_row_now] & ((uint16)1 << (i + 1)))
                    {
                ...
                for(; 10 > i; i++)
                {
                    if(dot_matrix_screen_bitmap_backup[1][display_row_now] & ((uint16)1 << (i - 5 + 1)))
                    {
                        tld7002_duty[i] = dot_matrix_screen_brightness;
                    }
                    else
                ...
                for(; 15 > i; i++)
                {
                    if(dot_matrix_screen_bitmap_backup[2][display_row_now] & ((uint16)1 << (i - 10 + 1)))
                    {
                        tld7002_duty[i] = dot_matrix_screen_brightness;
                    }
                    else
...
{
    uint8 i;
    dot_matrix_screen_show_bitmap_flag = 0;
    for(i = 0; i < 3; i++)
    {
        if(0 == str[i])
        {
...
//-------------------------------------------------------------------------------------------------------------------
//  函数简介      点阵屏自定义位图显示函数
//  参数说明      bitmap        [3][7]，每个字位 5x7 点阵，bit1~bit5 为 5 列像素（bit0 不使用）
//  返回参数      void
//  使用示例      dot_matrix_screen_show_bitmap(my_bitmap);
//-------------------------------------------------------------------------------------------------------------------
void dot_matrix_screen_show_bitmap(const uint8 bitmap[3][7])
{
    dot_matrix_screen_show_bitmap_flag = 1;
    memcpy((uint8 *)dot_matrix_screen_bitmap, (uint8 *)bitmap, sizeof(dot_matrix_screen_bitmap));
}
...
//点阵屏显示双闪图案
void dot_matrix_screen_shuangshan(void)
{
    dot_matrix_screen_show_bitmap(shuang_shan);
}

//点阵屏显示左转图案
void dot_matrix_screen_zuozhuan(void)
{
    dot_matrix_screen_show_bitmap(zuo_zhuan);
}

//点阵屏显示右转图案
void dot_matrix_screen_youzhuan(void)
{    
    dot_matrix_screen_show_bitmap(you_zhuan);
}

//点阵屏显示近光图案
void dot_matrix_screen_jinguang(void)
{    dot_matrix_screen_show_bitmap(jin_guang);
}  

//点阵屏显示远光图案
void dot_matrix_screen_yuanguang(void)
{    dot_matrix_screen_show_bitmap(yuan_guang);
}

//点阵屏显示雾灯图案
void dot_matrix_screen_wudeng(void)
{    dot_matrix_screen_show_bitmap(wu_deng);}

//点阵屏显示车内灯图案
void dot_matrix_screen_cheneideng(void)
{    dot_matrix_screen_show_bitmap(che_neideng);}

//点阵屏显示雨刷器图案
void dot_matrix_screen_yuguaqi(void)
{    dot_matrix_screen_show_bitmap(yu_guaqi);}
